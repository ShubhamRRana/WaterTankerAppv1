...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (plan.account_kind && plan.account_kind !== "agency") {
      return errorResponse("Plan is not an agency subscription plan", 400);
    }

    const amountPaise = rupeesToPaise(Number(plan.price));
    const currency = (plan.currency as string) || "INR";
    const receipt = `agency_sub_${subscriptionId.slice(0, 8)}_${Date.now()}`;
