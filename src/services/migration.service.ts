        ...
        try {
          ...
          if (createAuthAccounts && localUser.password) {
            try {
              ...
              if (authError) {
                // If user already exists in Auth, skip creating auth account
                // The user will need to be linked manually or via admin panel
                if (authError.message.includes('already registered') || authError.message.includes('already exists')) {
                  warnings.push(`Auth account already exists for ${localUser.phone}, skipping auth creation`);
                  // Continue without auth account - user can be linked manually
                } else {
                  throw authError;
                }
              ...
              warnings.push(`Failed to create Auth account for ${localUser.phone}: ${authErr.message}`);
              // Continue without auth account - user can be created manually
            }
          }
